    ...
    public static class Utils
    {
        ...
        public static DateTimeOffset GetCurrentTime()
        {
            using (var client = new HttpClient())
            {
                ...
                return (DateTimeOffset)result.Headers.Date;

            }
        }

        public static float getDateSubscration(int subscriptionDate)
        {

            var baseDate = new DateTime(1970, 01, 01);
            DateTime date = GetCurrentTime().DateTime;


            var numberOfSeconds = date.Subtract(baseDate).TotalSeconds;
            var diff = numberOfSeconds - subscriptionDate;

            return ((float)(diff / 60 / 60 / 24));
        }


        public static float  getDateSubscrationSeconds(int loginDate)
        {
            var baseDate = new DateTime(1970, 01, 01);
            DateTime date = GetCurrentTime().DateTime;
            var numberOfSeconds = date.Subtract(baseDate).TotalSeconds;
            var diff = numberOfSeconds - loginDate;
